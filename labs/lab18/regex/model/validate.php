...
class Validate {
    ...
    public function birthdate($name, $value) {
        ...
        if (!$field->hasError() && !$field->isEmpty()) {
            ...
            $message = 'Invalid birth date. Please use mm/dd/yyyy format.';
            $this->pattern($name, $value, $pattern, $message);
            ...
            if(!$field->hasError()) {
                ...
                if ($birthdate > $today) {
                    $field->setErrorMessage("Invalid birth date, unless you're from the future.");
                } else {
                    $field->clearErrorMessage();
                }
            }
    ...
    public function password($name, $password) {
        ...
        if (!$field->hasError() && !$field->isEmpty()) {
            ...
            $pattern = "/^(?=.*[[:digit:]])(?=.*[[:upper:]])[[:digit:][:upper:][:lower:]]{8,}$/";
            $message = 'Must have one each of Upper and Digit';
